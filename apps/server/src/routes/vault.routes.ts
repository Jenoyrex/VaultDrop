...
export function createVaultRouter(
  ...
  const createVaultSchema = z.object({
    ...
    // Required as of Phase 1: every new vault must be created encrypted,
    // so the full envelope must be present. The server treats every field
    // here as an opaque, already-encrypted blob generated by the browser
    // — it never derives or sees the KEK/DEK/password. (Vaults created
    // before this requirement may still exist as plaintext in the
    // database; this only closes the creation path.)
